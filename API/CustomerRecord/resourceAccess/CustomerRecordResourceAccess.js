...
const { CHECKING_STATUS, DATE_DB_FORMAT, DATE_DB_SORT_FORMAT } = require('../CustomerRecordConstants');
...
function _makeQueryBuilderByFilterByExpiredDate(filter, skip, limit, startDate, endDate, searchText, order) {
  ...
  if (startDate) {
    queryBuilder.where('customerRecordCheckExpiredDay', '>=', startDate);
  }

  if (endDate) {
    queryBuilder.where('customerRecordCheckExpiredDay', '<=', endDate);
  }
  if (filterData.returnNumberCount) {
    queryBuilder.where('returnNumberCount', '>=', filterData.returnNumberCount);
...
async function customSearch(filter, skip, limit, startDate, endDate, searchText, order) {
  if (startDate) {
    let momentVal = moment(startDate, DATE_DB_FORMAT).hours(0).minutes(0).toISOString();
    momentVal = momentVal.split('T');
    momentVal[1] = '00:00:01.000Z';
    startDate = momentVal.join('T');
  }
  if (endDate) {
    let momentVal = moment(endDate, DATE_DB_FORMAT).hours(23).minutes(59).toISOString();
    momentVal = momentVal.split('T');
    momentVal[1] = '23:59:59.000Z';
    endDate = moment(endDate, DATE_DB_FORMAT).hours(23).minutes(59).toISOString();
  }
...
async function customCount(filter, startDate, endDate, searchText, order) {
  if (startDate) {
    startDate = moment(startDate, DATE_DB_FORMAT).hours(0).minutes(0).toDate();
  }
  if (endDate) {
    endDate = moment(endDate, DATE_DB_FORMAT).hours(23).minutes(59).toDate();
  }
...
async function customSearchByExpiredDate(filter, skip, limit, startDate, endDate, searchText, order) {
  if (startDate) {
    startDate = moment(startDate, DATE_DB_FORMAT).format(DATE_DB_SORT_FORMAT) * 1;
  }
  if (endDate) {
    endDate = moment(endDate, DATE_DB_FORMAT).format(DATE_DB_SORT_FORMAT) * 1;
  }
