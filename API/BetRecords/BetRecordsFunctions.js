...
async function _checkWinLoseForBetByGame(betRecord, game) {
  ...
  if ((!game.gameRecordResult || game.gameRecordResult === null || game.gameRecordResult === "") &&
  ...
  let _updatedRecord = await BetRecordsResource.updateById(_detailRecord.betRecordId, {
...
async function checkWinLoseForAllBetByGame(gameRecordId) {
  ...
  let _countAllBetOfGame = await BetRecordsResource.count({
  ...
  for (let batchCounter = 0; batchCounter < _countAllBetOfGame; batchCounter++) {
    let batchBetRecords = await BetRecordsResource.find({
      gameRecordId: game.gameRecordId,
      betRecordStatus: BET_STATUS.NEW
    }, batchCounter * 100, 100);
    let _promiseList = [];

    if (batchBetRecords && batchBetRecords.length > 0) {
      for (let recordCounter = 0; recordCounter < batchBetRecords.length; recordCounter++) {
        const _record = batchBetRecords[recordCounter];
...
async function sumBetRecordBonusForUser(appUserId) {
  let totalBonus = await BetRecordsResource.sum('betRecordAmountIn', {
    appUserId: appUserId,
    betRecordStatus: BET_STATUS.COMPLETED,
...
async function updateAllBonusPaymentStatusByUser(appUserId) {
  let updateResult = await BetRecordsResource.updateAll({
