...
const tableName = 'PaymentBonusTransactionReferUserView';
...
async function createView() {
  ...
  var viewDefinition = DB.select(fields)
    ...
    .leftJoin(UserTableName, function () {
      this.on(`${rootTableName}.referUserId`, '=', `${UserTableName}.appUserId`);
    });
  for (let i = 0; i < sumFieldsList.length; i++) {
    viewDefinition.sum(`${sumFieldsList[i]} as ${sumFieldsList[i]}`);
  }
