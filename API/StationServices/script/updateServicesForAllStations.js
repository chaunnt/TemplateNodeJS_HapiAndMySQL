...
const { addDefaultServicesForStations } = require('../StationServicesFunctions');

async function updateServicesForAllStations() {
  console.info(`START updateServicesForAllStations ${new Date()}`);
  ...
  for (let i = 0; i < stationsListCount; i++) {
    ...
    await addDefaultServicesForStations(stationsId);
  }
  console.info(`FINISH updateServicesForAllStations ${new Date()}`);
}

updateServicesForAllStations();
