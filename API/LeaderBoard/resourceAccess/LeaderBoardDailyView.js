...
const tableName = 'LeaderBoardDailyViews';
const rootTableName = 'LeaderBoardDaily';
...
async function createView() {
  let fields = [
    ...
    `${rootTableName}.leaderBoardDailyDate`,
    `${rootTableName}.dateTime`,
...
async function customSumForRanking(filter, startDate, endDate, sumField, sumFieldExtra, skip, limit, order) {
  const moment = require('moment');
  let query = await DB(tableName)
    ...
    .select('firstName')
    .where(filter)
    .where('dateTime', '>=', moment(startDate, 'DD/MM/YYYY').format('YYYYMMDD') * 1)
    .where('dateTime', '<=', moment(endDate, 'DD/MM/YYYY').format('YYYYMMDD') * 1)
    .sum(`${sumField} as ${sumField}`)
    .sum(`${sumFieldExtra} as ${sumFieldExtra}`)
    .groupBy('appUserId')
    .groupBy('username')
    .groupBy('lastName')
    .groupBy('firstName')
    .count()
    .orderBy(sumField, order.value)
    .offset(skip)
    .limit(limit);
  return query;
}

async function customCountForRanking(filter, startDate, endDate, sumField, sumFieldExtra, skip, limit, order) {
  const moment = require('moment');
  let query = await DB(tableName)
    .where(filter)
    .where('dateTime', '>=', moment(startDate, 'DD/MM/YYYY').format('YYYYMMDD') * 1)
