...
async function addNewGameSection() {
  ...
  try {
    ...
    for (let index = 0; index < 10; index++) {
      ...
      newGameSection += `-${GAME_ID.BAUCUA1P}`;
      let gameRecordValue = null; //cronjob tạo các bản ghi game record với giá trị null
      let result = await GameFunction.addNewGameRecord(newGameSection, BET_TYPE.BAUCUA1P, gameRecordValue, false);
      if (result) {
        resultData.push(result);
      } else {
        Logger.info(`[Failure]: ${newGameSection}-${index + 1} -  Inserted baucua game record failure`);
      }
    }
  } catch (error) {
    Logger.error(`[Error]: ${currentSection} - Inserted baucua game record error`, error);
  }
}
