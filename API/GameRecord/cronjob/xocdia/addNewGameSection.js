...
async function addNewGameSection() {
  ...
  try {
    ...
    for (let index = 0; index < 10; index++) {
      ...
      newGameSection += `-${GAME_ID.XOCDIA1P}`;
      let gameRecordValue = null; //cronjob tạo các bản ghi game record với giá trị null
      let result = await GameFunction.addNewGameRecord(newGameSection, BET_TYPE.XOCDIA1P, gameRecordValue, false);
      if (result) {
    ...
    Logger.error(`[Error]: ${currentSection} - Inserted xocdia game record error`, error);
  }
}
