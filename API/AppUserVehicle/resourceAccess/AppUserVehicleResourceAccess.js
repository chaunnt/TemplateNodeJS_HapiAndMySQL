    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          ...
          table.integer('vehicleSubType').nullable(); // loai phuong tien - phan loai chi tiet
          ...
          table.string('vehicleExpiryDay'); // ngay het han // MỚI // Sử dụng để lọc
          table.string('certificateSeries'); // so GCN moi nhat
          table.string('extendLicenseOriginUrl', 500); // giay gia han ben cuc
          table.string('extendLicenseUrl', 500);
          table.integer('vehicleVerifiedInfo').defaultTo(VERIFICATION_STATUS.NOT_VERIFIED); // xac thuc phuon tien
