    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          ...
          table.integer('messageSendStatus').defaultTo(MARKETING_MESSAGE_SEND_STATUS.NEW);
          table.integer('customerMessageCategories');
          table.integer('messageFCMStatus').defaultTo(MARKETING_MESSAGE_SEND_STATUS.NEW);
          ...
          table.string('messageNote', 1000).nullable();
          table.text('messageTemplateData').nullable();
          table.string('messageZNSTemplateId').nullable();
          table.integer('messageTemplateId').nullable();
          table.integer('retryZaloToSMS').nullable();
          table.string('customConfigZNS').nullable();
          timestamps(table);
...
function _makeQueryBuilderByFilter(filter, skip, limit, startDate, endDate, searchText, order) {
  ...
  if (searchText) {
    ...
    queryBuilder.where(function () {
      this.orWhere('messageContent', 'like', `%${searchText}%`).orWhere('customerMessagePhone', 'like', `%${searchText}%`);
    });
  }
