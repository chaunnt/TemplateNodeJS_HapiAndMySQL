...
const { SCHEDULE_STATUS, SCHEDULE_TYPE } = require('../CustomerScheduleConstants');
...
describe(`Tests ${Model.modelName}`, function () {
  ...
  it('Check error UNCONFIRMED_BOOKING_EXISTED', done => {
    chai
      .request(`0.0.0.0:${process.env.PORT}`)
      .post(`/CustomerSchedule/user/createSchedule`)
      .set('Authorization', `Bearer ${token}`)
      ...
      .end((err, res) => {
        ...
        CheckError_UNCONFIRMED_BOOKING_EXISTED(body).then(error => {
          if (error) {
            checkResponseStatus(res, 500);
            expect(err === 'UNCONFIRMED_BOOKING_EXISTED');
          }
        });
  ...
  it('Check error BOOKING_ON_TODAY', done => {
    chai
      .request(`0.0.0.0:${process.env.PORT}`)
      .post(`/CustomerSchedule/user/createSchedule`)
      .set('Authorization', `Bearer ${token}`)
      ...
      .end((err, res) => {
        ...
        if (CheckError_BOOKING_ON_TODAY(body)) {
          checkResponseStatus(res, 500);
          expect(err === 'BOOKING_ON_TODAY');
        }
  ...
  it('Check error INVALID_PLATE_NUMBER', done => {
    chai
      .request(`0.0.0.0:${process.env.PORT}`)
      .post(`/CustomerSchedule/user/createSchedule`)
      .set('Authorization', `Bearer ${token}`)
      ...
      .end((err, res) => {
        ...
        if (CheckError_INVALID_PLATE_NUMBER(body)) {
          checkResponseStatus(res, 500);
          expect(err === 'INVALID_PLATE_NUMBER');
        }
