...
const { VEHICLE_FILE_TYPE } = require('../VehicleProfileConstants');
const tableName = 'VehicleFile';
const primaryKeyField = 'vehicleFileId';
    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          ...
          table.integer('vehicleProfileId');
          table.string('vehicleFileName').nullable();
          table.string('vehicleFileUrl', 500);
          table.integer('vehicleFileType').defaultTo(VEHICLE_FILE_TYPE.IMAGE);
          timestamps(table);
          table.index('vehicleProfileId');
        })
        .then(async () => {
