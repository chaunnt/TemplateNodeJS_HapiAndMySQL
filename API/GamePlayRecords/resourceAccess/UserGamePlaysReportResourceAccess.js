...
const tableName = 'UserGamePlaysReport';
    ...
    DB.schema.dropTableIfExists(`${tableName}`).then(() => {
      DB.schema
        .createTable(`${tableName}`, function (table) {
          ...
          table.integer('totalF1UserCount').defaultTo(0); // tổng F1
          table.integer('totalSystemUserCount').defaultTo(0); // tổng hệ thống
          table.integer('totalBetRecordAmountIn').defaultTo(0); // KLGD cá nhân
          table.integer('sumSystemBetAmountIn').defaultTo(0); // KLGD hệ thống
          timestamps(table);
        })
        .then(() => {
