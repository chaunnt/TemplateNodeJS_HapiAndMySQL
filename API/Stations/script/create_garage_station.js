...
const StationsManager = require('../manager/StationsManager');

async function seedStationsGarage() {
  const { GARAGE_STATIONS } = require('../data/GarageStation');
  const { STATIONS_AREA } = require('../data/StationsArea');

  const stationList = GARAGE_STATIONS;
  ...
  for (let i = 0; i < stationList.length; i++) {
    ...
    let _newStationData = {
      ...
      stationType: _station.stationType,
